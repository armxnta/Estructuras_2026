...
{
	Pila aux1 = {NULL,0,-1,pila->imprimir,pila->liberar};
	Pila aux2 = {NULL,0,-1,pila->imprimir,pila->liberar};
	...
	while(!pilaVacia(*pila))
	{
		...
		{
			cimaAux = aux1.cima->dato;
			
			if(comparar(cimaAux, temp) < 0)
			{
				pushDato(&aux2, popDato(&aux1));
			}
			else
			{
				break;
		...
		{
			pushDato(&aux1, popDato(&aux2));
		}
	}
	
	while(!pilaVacia(aux1))
	{
		pushDato(pila, popDato(&aux1));
	}
}
